...
const Input: React.FC<InputProps> = ({ label, className, value, ...props }) => {
  // FIXED: Bumped height to the "sweet spot" of 56px (h-[56px])
  const inputBaseStyles = "peer block w-full h-[56px] px-4 pt-5 pb-1.5 text-[14px] font-bold text-gray-900 bg-white border border-gray-200 rounded-xl appearance-none transition-all outline-none focus:ring-0 focus:border-brand-green disabled:opacity-50 disabled:cursor-not-allowed";
