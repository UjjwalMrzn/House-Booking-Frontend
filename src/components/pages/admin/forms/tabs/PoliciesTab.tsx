    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] overflow-hidden animate-entrance">
        <div className="p-4 md:p-6 border-b border-gray-50 bg-white/50 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          {checkInOutRules.length === 0 && !isViewMode && (
            <Button onClick={() => setRuleForm({ isOpen: true, isEdit: false, id: null, check_in: '14:00', check_out: '11:00' })} className="w-full sm:w-auto px-4 py-2 text-xs flex items-center justify-center gap-2 bg-amber-500 hover:bg-amber-600 shadow-amber-500/30">
          ...
          {checkInOutRules.length === 0 ? (
            ...
            <div className="grid grid-cols-1 gap-3">
              {checkInOutRules.map((rule: any, index: number) => (
                <div key={index} className="flex flex-col xs:flex-row items-center justify-between p-4 rounded-xl border border-amber-100 bg-amber-50/30 gap-4">
                  <div className="flex gap-6 sm:gap-10 self-start xs:self-auto">
                    <div>
      ...
      <div className="bg-white rounded-[2rem] border border-gray-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] overflow-hidden animate-entrance" style={{ animationDelay: '0.1s' }}>
        <div className="p-4 md:p-6 border-b border-gray-50 bg-white/50 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          {!isViewMode && (
          <Button onClick={() => setPolicyForm({ isOpen: true, isEdit: false, id: null, name: '', description: '' })} className="w-full sm:w-auto px-4 py-2 text-xs flex items-center justify-center gap-2">
          ...
          {policies.length === 0 ? (
            ...
            <div className="space-y-3">
              {policies.map((policy: any, index: number) => (
                <div key={index} className="flex items-start justify-between p-4 rounded-xl border border-gray-100 bg-white hover:border-gray-200 transition-all shadow-sm">
                  <div className="flex gap-4 min-w-0">
                    <div className="w-10 h-10 mt-1 bg-gray-50 text-gray-400 rounded-lg flex items-center justify-center shrink-0">
                      <FileText size={18} />
                    </div>
                    <div className="min-w-0">
                      <h4 className="text-sm font-black text-brand-dark truncate">{policy.name}</h4>
                      <p className="text-xs font-normal text-gray-400 mt-1 leading-relaxed">{policy.description}</p>
                    </div>
                  </div>
                  {!isViewMode && (
      ...
      >
        {/* SURGICAL FIX: Stacking inputs on mobile (grid-cols-1) to allow wide dropdowns to safely open */}
        ...
        {/* SURGICAL FIX: Added margin-top to separate button from the stacked inputs */}
        <Button onClick={() => saveRuleMutation.mutate(ruleForm)} disabled={!ruleForm.check_in || !ruleForm.check_out || saveRuleMutation.isPending} className="w-full py-3.5 bg-amber-500 hover:bg-amber-600 shadow-amber-500/30 text-white border-none mt-4">
      ...
      >
        ...
        <div className="space-y-1.5 relative group pb-4 mt-4">
          <div className="flex justify-between items-end">
            <label className="text-[11px] font-black uppercase tracking-widest text-gray-400">Policy Details</label>
            <span className="text-[9px] font-bold text-amber-500 uppercase tracking-widest">Max 250 Chars</span>
          </div>
          <textarea 
            value={policyForm.description} 
            onChange={(e) => setPolicyForm({ ...policyForm, description: e.target.value })} 
            placeholder="No unapproved visitors allowed." 
            maxLength={250} 
            rows={4} 
          ...
          {!isViewMode && (
            <div className="absolute bottom-0 right-2 text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
              <span className={policyForm.description?.length >= 250 ? "text-red-500" : ""}>{policyForm.description?.length || 0}</span> / 250
            </div>
          )}
        </div>
        <Button onClick={() => savePolicyMutation.mutate(policyForm)} disabled={!policyForm.name || !policyForm.description || savePolicyMutation.isPending} className="w-full py-3.5 shadow-[0_8px_15px_-5px_rgba(74,222,128,0.4)] mt-2">
      ...
      <Modal 
        ...
        title="Remove Times"
        message="Are you sure you want to remove the arrival and departure times?"
      ...
      <Modal 
        ...
        title="Delete Policy"
        message={`Are you sure you want to delete "${deletePolicyModal.name}"?`}
