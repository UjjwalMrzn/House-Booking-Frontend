...
const AdminBookingsPage = () => {
  ...
  const bookings = useMemo(() => {
    ...
    return Array.isArray(paginatedData)
      ? paginatedData
  ...
  const statusMutation = useMutation({
    ...
    onSuccess: (_: any, variables: { id: number; status: string }) => {
      ...
      if (viewModal.isOpen && viewModal.booking?.id === variables.id) {
        setViewModal((prev) => ({
          ...prev,
          booking: { ...prev.booking, status: variables.status },
        }));
      }
    },
      ...
      result.sort((a, b) => {
        if (sortConfig.key === "status") {
          ...
          return sortConfig.direction === "asc"
            ? (priority[a.status?.toLowerCase() || ""] || 99) - (priority[b.status?.toLowerCase() || ""] || 99)
            : (priority[b.status?.toLowerCase() || ""] || 99) - (priority[a.status?.toLowerCase() || ""] || 99);
        }
        let aValue = a[sortConfig.key];
        let bValue = b[sortConfig.key];
        if (sortConfig.key === "total_price" || sortConfig.key === "guests" || sortConfig.key === "id") {
  ...
  const renderStatusBadge = (status: string) => {
    switch (status?.toLowerCase()) {
      case "confirmed":
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-brand-green/10 text-brand-green text-[10px] font-black uppercase tracking-widest"><CheckCircle size={12} strokeWidth={3} /> Confirmed</span>;
      case "cancelled":
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-red-500/10 text-red-500 text-[10px] font-black uppercase tracking-widest"><XCircle size={12} strokeWidth={3} /> Cancelled</span>;
      default:
        return <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-500/10 text-amber-500 text-[10px] font-black uppercase tracking-widest"><Clock size={12} strokeWidth={3} /> Pending</span>;
    }
  };
          ...
          <table className="w-full text-left border-separate border-spacing-y-2 min-w-full lg:min-w-[950px] px-2 md:px-4">
            <thead>
              <tr>
                ...
                <th className="py-2 px-4"><button onClick={() => handleSort("total_price")} className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-gray-400">Amount {renderSortIcon("total_price")}</button></th>
          ...
          {viewModal.booking && (
            <div className="space-y-6">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                <div><p className="text-[10px] font-black uppercase tracking-widest text-gray-400">BOOKING ID</p><p className="text-xl md:text-2xl font-black text-brand-dark mt-1">#{viewModal.booking.id}</p></div>
                <div className="shrink-0">{renderStatusBadge(viewModal.booking.status)}</div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                  <div className="flex items-center gap-2 mb-2 text-gray-400"><Clock size={14} /><p className="text-[10px] font-black uppercase tracking-widest">Booked At</p></div>
                  <p className="text-sm font-bold text-brand-dark truncate">{viewModal.booking.createdAt ? new Date(viewModal.booking.createdAt).toLocaleDateString() : 'N/A'}</p>
                </div>
              </div>
              <div className="p-4 bg-brand-green/5 rounded-xl border border-brand-green/10 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-[10px] font-black uppercase tracking-widest text-brand-green">Guests</p><p className="text-sm font-black text-brand-dark">{viewModal.booking.adults !== undefined ? `${viewModal.booking.adults} Adults, ${viewModal.booking.kids || 0} Kids` : `${viewModal.booking.guests} People`}</p></div>
                </div>
              </div>
              <div className="border border-gray-100 rounded-xl overflow-hidden">
                <div className="bg-gray-50 px-4 py-2.5 border-b border-gray-100"><p className="text-[10px] font-black uppercase tracking-widest text-gray-500">Contact Details</p></div>
                <div className="p-5 bg-white grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4">
                  <div><p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Name</p><p className="text-sm font-medium text-brand-dark truncate">{isLoadingCustomer ? "Loading..." : customerData ? `${customerData.firstName} ${customerData.lastName}` : "Not provided"}</p></div>
                  <div><p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Email</p><p className="text-sm font-medium text-brand-dark truncate">{isLoadingCustomer ? "Loading..." : customerData?.email || "Not provided"}</p></div>
                  <div><p className="text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-1">Phone</p><p className="text-sm font-medium text-brand-dark truncate">{isLoadingCustomer ? "Loading..." : customerData?.phoneNumber || "Not provided"}</p></div>
              ...
              <div className="flex justify-between items-center pt-2">
                <div className="flex items-center gap-2 text-gray-400"><DollarSign size={16} /><p className="text-[10px] font-black uppercase tracking-widest">Total Amount</p></div>
                <p className="text-2xl md:text-3xl font-black text-brand-green">${Number(viewModal.booking.total_price).toLocaleString()}</p>
              </div>
              {viewModal.booking.status?.toLowerCase() === "pending" && (
                <div className="pt-2 flex gap-3">
                  <button onClick={() => { setViewModal(p => ({ ...p, isOpen: false })); setTimeout(() => setActionModal({ isOpen: true, type: "cancel", bookingId: viewModal.booking.id }), 200); }} className="flex-1 py-3 rounded-xl font-bold text-gray-500 bg-white border border-gray-200 hover:bg-red-50 hover:text-red-500 transition-colors">Reject</button>
                  <button onClick={() => { setViewModal(p => ({ ...p, isOpen: false })); setTimeout(() => setActionModal({ isOpen: true, type: "confirm", bookingId: viewModal.booking.id }), 200); }} className="flex-1 py-3 rounded-xl font-bold text-white bg-brand-green hover:bg-emerald-600 transition-colors">Approve</button>
                </div>
              )}
            </div>
          )}
